...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    const { data: recent } = await supabase
      .from('tours')
      .select('*, customers(item_id, guest_name)')
      .eq('status', 'active')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/" className="btn btn-outline-primary">← Voucher Management</Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="stat-card">
          <div className="stat-icon bg-[var(--color-info-light)] text-[var(--color-info)]">🗺️</div>
          <div><div className="stat-label">Active Tours</div><div className="stat-value">{stats.tours}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon bg-[var(--color-purple-light)] text-[var(--color-purple)]">🏨</div>
          <div><div className="stat-label">Active Hotels</div><div className="stat-value">{stats.hotels}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon bg-[var(--color-success-light)] text-[var(--color-success)]">💰</div>
          <div><div className="stat-label">Revenue</div><div className="stat-value">฿{Math.round(stats.revenue).toLocaleString()}</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon bg-[var(--color-warning-light)] text-[var(--color-warning)]">💵</div>
          <div><div className="stat-label">Profit</div><div className="stat-value">฿{Math.round(stats.profit).toLocaleString()}</div></div>
        </div>
      </div>
      ...
      <div className="card p-6">
        ...
        {recentTours.length === 0 ? (
          <p className="text-[var(--color-text-muted)] text-sm py-8 text-center">ยังไม่มีรายการ</p>
        ) : (
              ...
              <tbody>
                {recentTours.map(t => (
                  <tr key={t.id}>
                    ...
                    <td>{(t.adult || 0) + (t.child || 0)}</td>
                    <td>฿{(parseFloat(t.sale_amount) || 0).toLocaleString()}</td>
                    <td>
                      <Link href={`/customers/${t.customers?.item_id}`} className="text-[var(--color-brand)] text-sm font-medium hover:underline">View →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
