...
export default function DatabasePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><h2 className="text-2xl font-bold">📋 ฐานข้อมูล Dropdown</h2><Link href="/" className="btn btn-outline-primary">← กลับหน้าหลัก</Link></div>
      <div className="card p-6">
        <div className="flex flex-wrap gap-2 mb-6">
          {CATEGORIES.map(c => (<button key={c.key} onClick={() => setActiveTab(c.key)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === c.key ? 'bg-[var(--color-brand)] text-white' : 'bg-[var(--color-surface-alt)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)]'}`}>{c.label}</button>))}
        </div>
        ...
        : <div className="space-y-2">{items.map((item, idx) => (
            <div key={item.id} className="flex items-center justify-between px-4 py-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)]">
              <div className="flex items-center gap-3"><span className="text-sm text-[var(--color-text-muted)] w-6">{idx + 1}.</span><span className="font-medium">{item.value}</span></div>
